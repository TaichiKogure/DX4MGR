...
class SimConfig:
    ...
    def from_dict(cls, data: dict):
        ...
        pybamm_data = data.get('pybamm')
        pybamm_cfg = None
        if pybamm_data:
            pybamm_cfg = PyBaMMConfig(
                parameter_set=pybamm_data.get('parameter_set', 'Chen2020'),
                custom_parameters=pybamm_data.get('custom_parameters')
            )
