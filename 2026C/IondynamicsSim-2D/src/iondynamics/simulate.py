...
from iondynamics.config import SimConfig
...
class SimResult:
    ...
    config: SimConfig
    # 厚み方向の端点インデックス (0: separator側, -1: current collector側 と定義)
    # DFNモデルの正極内座標は通常 セパレータ境界から集電体境界に向かう
    x_is_from_separator: bool = True 
...
def run_simulation(cfg: SimConfig) -> SimResult:
    ...
    # 2. パラメータ設定
    param_set = "Chen2020"
    if cfg.pybamm and cfg.pybamm.parameter_set:
        param_set = cfg.pybamm.parameter_set
    ...
    # カスタムパラメータがあれば上書き
    if cfg.pybamm and cfg.pybamm.custom_parameters:
        update_dict.update(cfg.pybamm.custom_parameters)
        
    params.update(update_dict, check_already_exists=False)
