...
class Simulation:
    ...
    def step(self):
        ...
        if total_evidence >= self.dr_threshold and not self.dr_passed:
            ...
            # 量産移行（TRL Gapの表現）
            # プロトタイプでの「証拠」が量産では一部通用しない
            for t in self.tech_items:
                t.uncertainty = min(1.0, t.uncertainty + 0.2) # TRL Gapによる不確実性の再燃
                t.evidence *= 0.7 # 証拠の有効性が減少
        ...
        if self.production_started and np.random.random() < 0.05:
            ...
            if target.uncertainty > 0.2:
                # 不確実性が残っていると市場クレームになりやすい
                self.kpis['market_complaints'] += 1
                # 市場からのフィードバックによる手戻り
                target.uncertainty = min(1.0, target.uncertainty + 0.3)
                # print(f"[{self.current_time}] Market Feedback: Issue found in {target.name}!")
