...
class Simulation:
    ...
    def setup_with_params(self, params):
        ...
        # 実測ログからのキャリブレーション (Plan 5 特有)
        if params.get('use_digital_twin'):
            self.calibrate_from_logs([research, prototype, analysis])
    ...
    def calibrate_from_logs(self, teams):
        """
        実過去データ（ELN/Jira等）からパラメータを推定
        """
        import pandas as pd
        import os
        ...
        if os.path.exists(log_path):
            ...
            for team in teams:
                ...
                if not team_data.empty:
                    # ログに基づいたスキル値の上書きや、エラー率の反映
                    team.skill = 1.0 - team_data['error_rate'].iloc[0]
                    # print(f"Calibrated {team.name} skill to {team.skill:.2f} from logs.")
    ...
    def step(self):
        ...
        for msg in list(proto_team.inbox):
            if msg['arrival_time'] <= self.current_time and msg.get('type') == 'experiment_request':
                ...
                # 実験実行
                exp = Experiment(msg['tech_items'], proto_team, [])
