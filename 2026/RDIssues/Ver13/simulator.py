...
def simulate_standard_flow(
    ...
    n_servers_mid=5,
    n_servers_fin=3,
    engineer_pool_size=10,
    hours_per_day_per_engineer=6.0,
    seed=None,
    sampling_interval=1.0,
    **kwargs
):
    """
    SMALL_EXP -> BUNDLE_SMALL -> DR1 -> MID_EXP -> BUNDLE_MID -> DR2 -> FIN_EXP -> BUNDLE_FIN -> DR3
    """
    ...
    engine = setup_standard_flow(
        ...
        bundle_size_small=bundle_size_small,
        mid_exp_duration=mid_exp_duration,
        fin_exp_duration=fin_exp_duration,
        bundle_size_mid=bundle_size_mid,
        bundle_size_fin=bundle_size_fin,
        dr1_period=dr1_period,
        dr2_period=dr2_period,
        dr3_period=dr3_period,
        dr1_capacity=dr1_capacity,
        dr2_capacity=dr2_capacity,
        ...
        dr2_rework_multiplier=dr2_rework_multiplier,
        ...
        n_new=n_new,
        n_servers_mid=n_servers_mid,
        n_servers_fin=n_servers_fin,
        **kwargs
    )

    # Ver13 added: Scheduler integration
    from core.planning import Scheduler
    if hasattr(engine, "dr_calendar"):
        scheduler = Scheduler(
            engine=engine,
            calendar=engine.dr_calendar,
            engineer_pool_size=int(engineer_pool_size),
            hours_per_day_per_engineer=float(hours_per_day_per_engineer)
        )
        scheduler.start(0.0)
