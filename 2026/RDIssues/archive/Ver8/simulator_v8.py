...
def simulate_standard_flow_v8(
    ...
    bundle_size=3,
    dr_period=90,
    dr_capacity=10,   # 現状はapproversのcapacity側に寄せるなら今後反映（ここでは温存）
    dr_quality=0.8,   # 同上（MeetingGateのqualityへ寄せるなら今後反映）
    ...
    """
    Ver8 標準フロー版（SMALL_EXP→PROTO→BUNDLE→DR_GATE）
    runner/adapters.py の setup_standard_flow をメイン採用する。
    """
...
def _single_trial(args):
    params, seed = args
    return simulate_standard_flow_v8(**params, seed=seed)

def run_monte_carlo_v8(n_trials=100, use_parallel=True, **params):
    """
    モンテカルロ（標準フロー版）
    seedは trial_id をそのまま使う
    """
