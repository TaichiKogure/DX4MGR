...
def _build_scenarios_from_doe(df_top: pd.DataFrame, template_df: pd.DataFrame, n_trials: int) -> pd.DataFrame:
    ...
    for i, row in enumerate(df_top.itertuples(), start=1):
        ...
        if "dr2_rework_multiplier" in template_df.columns and hasattr(row, "dr2_rework_multiplier"):
            r["dr2_rework_multiplier"] = float(row.dr2_rework_multiplier)
        if "dr1_period" in template_df.columns:
            r["dr1_period"] = float(row.dr_period)
        if "dr2_period" in template_df.columns:
            r["dr2_period"] = float(row.dr_period)
        if "dr3_period" in template_df.columns:
            r["dr3_period"] = float(row.dr_period)
        rows.append(r)
...
def run_doe(
    ...
    for sample in doe_samples:
        ...
        params["dr1_period"] = params["dr_period"]
        params["dr2_period"] = params["dr_period"]
        params["dr3_period"] = params["dr_period"]
    ...
    viz.plot_doe_analysis(doe_df, title="Ver12 DOE Sensitivity (exploration)")
