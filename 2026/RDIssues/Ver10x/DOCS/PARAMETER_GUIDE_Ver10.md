...
※ **dr_capacity / dr_quality**:
  - `scenarios.csv` には項目として存在しますが、Ver10x では上記の要員構成から自動計算されるため、**ここに入力された値は無視されます**。過去バージョンとの互換性のために残されています。

---

## 6. 手戻りと増殖 (Rework & Complexity)

DR で否認（または条件付き合格）された際、案件は「小実験」工程に差し戻されます。Ver10x ではこれを「タスクの増殖」として表現します。
