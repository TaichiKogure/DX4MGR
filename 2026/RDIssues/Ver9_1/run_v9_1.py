...
from simulator_v9_1 import run_monte_carlo_v9_1, latin_hypercube_sampling
from analyzer_v9_1 import AnalyzerV8
import visualizer_v9_1 as viz

# 出力ディレクトリ
OUT_DIR = os.path.join(CURRENT_DIR, "output")
os.makedirs(OUT_DIR, exist_ok=True)

def run_v9_1_pipeline():
    print("=== DX4MGR Ver9.1: 新アーキテクチャ・増殖モデル ===")
    ...
    for sample in doe_samples:
        ...
        # DOEは軽く1トライアル評価
        res = run_monte_carlo_v9_1(n_trials=1, use_parallel=False, **params)[0]
        doe_results.append({**sample, "throughput": res["summary"]["throughput"]})

    doe_df = pd.DataFrame(doe_results)
    viz.plot_doe_analysis(doe_df, title="Ver9.1 DOE 感度分析（新アーキテクチャ）")
    plt.savefig(os.path.join(OUT_DIR, "v9_1_step1_doe_analysis.png"))
    plt.close()
    ...
    for _, row in df_scenarios.iterrows():
        ...
        trials = run_monte_carlo_v9_1(n_trials=n_trials, use_parallel=True, **params)
        ...
        viz.plot_quality_gate_status(gate_res, title=f"Quality Gate: {name} (Ver9.1)")
        plt.savefig(os.path.join(OUT_DIR, f"v9_1_step2_gate_{name}.png"))
        plt.close()
    ...
    # 5. 可視化
    print("\n[Step 4: 可視化レポートの生成]")
    viz.plot_comparison_with_ci(comparison_summary, title="全シナリオ比較: スループット(Ver9.1新アーキテクチャ)")
    plt.savefig(os.path.join(OUT_DIR, "v9_1_step3_comparison_ci.png"))
    plt.close()

    viz.plot_wait_time_distribution(all_waits, title="Ver9.1 待ち時間分布比較")
    plt.savefig(os.path.join(OUT_DIR, "v9_1_step3_compare_violin.png"))
    plt.close()

    viz.plot_rework_distribution(all_reworks, title="Ver9.1 差し戻し回数分布比較")
    plt.savefig(os.path.join(OUT_DIR, "v9_1_step3_compare_reworks.png"))
    plt.close()

    viz.plot_gate_wait_heatmap(gate_stats_by_scenario, title="Ver9.1 ゲート別 平均待ち時間")
    plt.savefig(os.path.join(OUT_DIR, "v9_1_step4_gate_wait_heatmap.png"))
    plt.close()

    viz.plot_gate_wip_heatmap(avg_wip_by_node_by_scenario, title="Ver9.1 ゲート別 平均WIP")
    plt.savefig(os.path.join(OUT_DIR, "v9_1_step4_gate_wip_heatmap.png"))
    plt.close()
    ...
    }, "v9_1_final_analysis_report.json")
