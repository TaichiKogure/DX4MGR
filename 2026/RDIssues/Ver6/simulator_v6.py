...
def simulate_flow_v6(
    ...
    p_rework=0.1,
    rework_load_factor=0.5, # 差し戻し時の負荷増大係数
    skill_factor=1.0,
    ...
    """
    離散イベントシミュレーション (Discrete Event Simulation: DES) - Ver6
    """
    if seed is not None:
        random.seed(seed)
    ...
    rework_counts = [] # 各ジョブの差し戻し回数
    
    events = [] # (time, event_type, data)
    
    if arrival_vp_per_day > 0:
        first_arrival = random.expovariate(arrival_vp_per_day)
        heapq.heappush(events, (first_arrival, 0, {"arrival_time": first_arrival, "load_multiplier": 1.0, "reworks": 0}))
    ...
    while events:
        ...
        if event_type == 0: # 到着
            ...
            load_mult = data["load_multiplier"]
            reworks = data["reworks"]
            
            # 次の新規到着を予約
            next_arrival_t = event_t + random.expovariate(arrival_vp_per_day)
            heapq.heappush(events, (next_arrival_t, 0, {"arrival_time": next_arrival_t, "load_multiplier": 1.0, "reworks": 0}))
            
            job_data = {"arrival_time": arrival_t, "load_multiplier": load_mult, "reworks": reworks}
            
            if len(approver_available) > 0:
                free_time = heapq.heappop(approver_available)
                start_time = max(event_t, free_time)
                # サービス時間は負荷係数に比例
                service_rate = effective_service_rate / load_mult if load_mult > 0 else effective_service_rate
                service_time = random.expovariate(service_rate) if service_rate > 0 else 1e9
                finish_time = start_time + service_time
                heapq.heappush(events, (finish_time, 1, {**job_data, "start_time": start_time}))
            else:
                waiting_queue.append(job_data)
            ...
            if random.random() < p_rework:
                # 差し戻し発生：負荷が増える（小実験が増える）
                new_load_mult = load_mult + rework_load_factor
                new_job_data = {"arrival_time": arrival_t, "load_multiplier": new_load_mult, "reworks": reworks + 1}
                # キューの先頭に戻す（優先的に再処理されることが多い）
                waiting_queue.insert(0, new_job_data)
            else:
                # 完了
                wait_time = data["start_time"] - arrival_t
                lead_time = event_t - arrival_t
                wait_times.append(wait_time)
                lead_times.append(lead_time)
                rework_counts.append(reworks)
                current_wip -= 1
            
            if waiting_queue:
                next_job = waiting_queue.pop(0)
                start_time = event_t 
                l_mult = next_job["load_multiplier"]
                service_rate = effective_service_rate / l_mult if l_mult > 0 else effective_service_rate
                service_time = random.expovariate(service_rate) if service_rate > 0 else 1e9
                finish_time = start_time + service_time
                heapq.heappush(events, (finish_time, 1, {**next_job, "start_time": start_time}))
            else:
    ...
    summary = {
        ...
        "avg_wip": np.mean([w for t, w in wip_log]) if wip_log else 0,
        "avg_reworks": np.mean(rework_counts) if rework_counts else 0,
        "max_reworks": np.max(rework_counts) if rework_counts else 0,
