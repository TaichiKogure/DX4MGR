...
from simulator_v9_2 import run_monte_carlo_v9_2, latin_hypercube_sampling
from analyzer_v9_2 import AnalyzerV8
import visualizer_v9_2 as viz

# 出力ディレクトリ
OUT_DIR = os.path.join(CURRENT_DIR, "output")
os.makedirs(OUT_DIR, exist_ok=True)

def run_v9_2_pipeline():
    print("=== DX4MGR Ver9.2: 承認者エージェント＆詳細観測モデル ===")
    ...
    for sample in doe_samples:
        ...
        # DOEは軽く1トライアル評価
        res = run_monte_carlo_v9_2(n_trials=1, use_parallel=False, **params)[0]
        doe_results.append({**sample, "throughput": res["summary"]["throughput"]})

    doe_df = pd.DataFrame(doe_results)
    viz.plot_doe_analysis(doe_df, title="Ver9.2 DOE 感度分析（承認者エージェント）")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step1_doe_analysis.png"))
    plt.close()
    ...
    for _, row in df_scenarios.iterrows():
        ...
        trials = run_monte_carlo_v9_2(n_trials=n_trials, use_parallel=True, **params)
        ...
        viz.plot_quality_gate_status(gate_res, title=f"Quality Gate: {name} (Ver9.2)")
        plt.savefig(os.path.join(OUT_DIR, f"v9_2_step2_gate_{name}.png"))
        plt.close()
    ...
    # 5. 可視化
    print("\n[Step 4: 可視化レポートの生成]")
    viz.plot_comparison_with_ci(comparison_summary, title="全シナリオ比較: スループット(Ver9.2)")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step3_comparison_ci.png"))
    plt.close()

    viz.plot_wait_time_distribution(all_waits, title="Ver9.2 待ち時間分布比較")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step3_compare_violin.png"))
    plt.close()

    viz.plot_rework_distribution(all_reworks, title="Ver9.2 差し戻し回数分布比較")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step3_compare_reworks.png"))
    plt.close()

    # Step 7: 新規可視化
    viz.plot_ccdf(all_waits, title="Ver9.2 超過確率カーブ (CCDF)")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step3_ccdf.png"))
    plt.close()

    viz.plot_wip_time_series(all_wip_histories, title="Ver9.2 WIP時系列推移")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step3_wip_time_series.png"))
    plt.close()

    viz.plot_rework_weight_distribution(all_rework_weights, title="Ver9.2 差し戻し重み分布")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step3_rework_weight_dist.png"))
    plt.close()

    viz.plot_proliferated_tasks_distribution(all_proliferated, title="Ver9.2 増殖タスク数分布")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step3_proliferated_dist.png"))
    plt.close()

    viz.plot_gate_wait_heatmap(gate_stats_by_scenario, title="Ver9.2 ゲート別 平均待ち時間")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step4_gate_wait_heatmap.png"))
    plt.close()

    viz.plot_gate_wip_heatmap(avg_wip_by_node_by_scenario, title="Ver9.2 ゲート別 平均WIP")
    plt.savefig(os.path.join(OUT_DIR, "v9_2_step4_gate_wip_heatmap.png"))
    plt.close()
    ...
    }, "v9_2_final_analysis_report.json")
