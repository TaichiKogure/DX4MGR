...
def plot_gate_wip_heatmap(avg_wip_by_node_by_scenario: dict, title="ゲート別 平均WIPヒートマップ"):
    ...
    plt.ylabel("Scenario")
    plt.tight_layout()
def plot_flow_time_breakdown(flow_df, node_order=None, title="Flow time breakdown"):
    if flow_df is None or flow_df.empty:
        return
    required = {"Scenario", "Node", "AvgTime"}
    if not required.issubset(set(flow_df.columns)):
        return

    pivot = flow_df.pivot_table(index="Scenario", columns="Node", values="AvgTime", aggfunc="mean").fillna(0.0)
    if node_order:
        cols = [c for c in node_order if c in pivot.columns]
        if cols:
            pivot = pivot[cols]

    fig, ax = plt.subplots(figsize=(12, max(4, 0.6 * pivot.shape[0])))
