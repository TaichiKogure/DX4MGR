...
from simulator_v6 import run_monte_carlo_v6, latin_hypercube_sampling, simulate_flow_v6
from analyzer_v6 import AnalyzerV6
import visualizer_v6 as viz

# 出力ディレクトリ
OUT_DIR = "2026/RDIssues/Ver6/output"
os.makedirs(OUT_DIR, exist_ok=True)

def run_v6_pipeline():
    print("=== DX4MGR Ver6: 差し戻し重み（負荷）考慮モデル ===")
    
    # 1. シナリオ読み込み
    csv_path = "2026/RDIssues/Ver6/scenarios.csv"
    if not os.path.exists(csv_path):
        print(f"Error: {csv_path} not found.")
        return
        
    df_scenarios = pd.read_csv(csv_path)
    analyzer = AnalyzerV6(OUT_DIR)
    ...
    for sample in doe_samples:
        ...
        res = simulate_flow_v6(**params, seed=42)
        doe_results.append({**sample, "throughput": res["summary"]["throughput"]})
    
    doe_df = pd.DataFrame(doe_results)
    viz.plot_doe_analysis(doe_df, title="Ver6 DOE 感度分析 (rework_load_factor 含む)")
    plt.savefig(f"{OUT_DIR}/v6_step1_doe_analysis.png")
    plt.close()
    ...
    for _, row in df_scenarios.iterrows():
        ...
        # モンテカルロ実行 (並列処理で高速化)
        trials = run_monte_carlo_v6(n_trials=n_trials, use_parallel=True, **params)
        ...
        all_reworks[name] = [r for t in trials for r in t["logs"]["rework_counts"]]
        ...
        plt.savefig(f"{OUT_DIR}/v6_step2_gate_{name}.png")
        plt.close()
    ...
    # 5. 全体可視化の生成
    print("\n[Step 4: 可視化レポートの生成]")
    viz.plot_comparison_with_ci(comparison_summary, title="全シナリオ比較: スループット(Ver6)")
    plt.savefig(f"{OUT_DIR}/v6_step3_comparison_ci.png")
    plt.close()
    
    viz.plot_wait_time_distribution(all_waits, title="Ver6 待ち時間分布比較")
    plt.savefig(f"{OUT_DIR}/v6_step3_compare_violin.png")
    plt.close()
    
    viz.plot_rework_distribution(all_reworks, title="Ver6 差し戻し回数分布比較")
    plt.savefig(f"{OUT_DIR}/v6_step3_compare_reworks.png")
    plt.close()
